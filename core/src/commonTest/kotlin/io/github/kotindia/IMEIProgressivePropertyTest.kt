...
/**
 * Property-based tests for [IMEI] progressive validation API.
 */
class IMEIProgressivePropertyTest {
    private val arbPartialDigits: Arb<String> =
        Arb.list(Arb.element(('0'..'9').toList()), 1..14).map { it.joinToString("") }
    ...
    fun pt2_sanitize_idempotent() {
        runBlocking {
            checkAll(1000, arbArbitraryShort) { input ->
                val once = IMEI.sanitize(input)
                assertEquals(once, IMEI.sanitize(once), "sanitize not idempotent: input=$input")
            }
        }
    }
    ...
    fun pt3_sanitize_boundsAndCharset() {
        runBlocking {
            checkAll(1000, arbArbitraryLong) { input ->
                val result = IMEI.sanitize(input)
                assertTrue(result.length <= IMEI.maxLength)
                for (c in result) assertTrue(c in IMEI.allowedChars, "disallowed char '$c' in: $result")
            }
        }
    }
}
