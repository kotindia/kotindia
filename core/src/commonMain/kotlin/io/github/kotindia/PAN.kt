...
// PAN accepts A-Z (uppercase) + a-z (lowercase, normalised) + 0-9.
// Non-contiguous ranges; must be Set<Char>, not CharRange.
private val PAN_ALLOWED_CHARS: Set<Char> =
    buildSet {
        for (c in 'A'..'Z') add(c)
        for (c in 'a'..'z') add(c)
        for (c in '0'..'9') add(c)
...
public object PAN {
    ...
    /**
     * Maximum accepted length for a sanitized PAN input.
     *
     * PAN is always 10 characters. Any sanitized input longer than this is over-length.
     */
    public val maxLength: Int = 10

    /**
     * Set of characters accepted by [sanitize] and [validateProgressive] for PAN.
     *
     * PAN accepts ASCII uppercase letters (`'A'..'Z'`), lowercase letters (`'a'..'z'`),
     * and decimal digits (`'0'..'9'`). Lowercase letters are treated as valid input and
     * normalised to uppercase by [validateProgressive]. Any other character is stripped by
     ...
     * directly to `if (char in PAN.allowedChars)` UI filters without pre-uppercasing. The
     ...
     */
    public val allowedChars: Set<Char> = PAN_ALLOWED_CHARS
     ...
     * @sample io.github.kotindia.samples.panSanitizeSample
     */
    public fun sanitize(rawInput: String): String = rawInput.filter { it in PAN_ALLOWED_CHARS }.take(maxLength)

    /**
     * Validates a PAN for incremental, as-you-type input.
     *
     ...
     *
     * @param value PAN input to evaluate, possibly partial. Callers should run [sanitize]
     ...
     * @sample io.github.kotindia.samples.panValidateProgressiveSample
     */
    public fun validateProgressive(value: String): ProgressiveResult {
        ...
        if (normalized.any { it !in PAN_ALLOWED_CHARS }) {
    ...
    // PAN has no standardised spacing — uppercase as-is.
