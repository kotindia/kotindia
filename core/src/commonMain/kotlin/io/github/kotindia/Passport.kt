...
private val PASSPORT_ALLOWED_CHARS: Set<Char> =
    buildSet {
        for (c in 'A'..'Z') add(c)
        for (c in 'a'..'z') add(c)
        for (c in '0'..'9') add(c)
...
public object Passport {
    ...
    /**
     * Maximum accepted length for a sanitized Passport input.
     *
     * Indian Passport numbers are always 8 characters. Any sanitized input longer than this is over-length.
     */
    public val maxLength: Int = 8

    /**
     * Set of characters accepted by [sanitize] and [validateProgressive] for Passport.
     *
     * Passport accepts ASCII uppercase letters (`'A'..'Z'`), lowercase letters (`'a'..'z'`),
     ...
     * directly to `if (char in Passport.allowedChars)` UI filters without pre-uppercasing. The
     * validator normalises internally via [validate] and [validateProgressive].
     *
     * Exposed as `Set<Char>` for uniform caller-side `if (char in Passport.allowedChars)` checks.
     * The constant is top-level to avoid per-call allocation.
     */
    public val allowedChars: Set<Char> = PASSPORT_ALLOWED_CHARS
     ...
     * @sample io.github.kotindia.samples.passportSanitizeSample
     */
    public fun sanitize(rawInput: String): String = rawInput.filter { it in PASSPORT_ALLOWED_CHARS }.take(maxLength)

    /**
     * Validates a Passport number for incremental, as-you-type input.
     *
     ...
     *
     * @param value Passport number input to evaluate, possibly partial. Callers should run [sanitize]
     ...
     * @sample io.github.kotindia.samples.passportValidateProgressiveSample
     */
    public fun validateProgressive(value: String): ProgressiveResult {
        ...
        if (normalized.any { it !in PASSPORT_ALLOWED_CHARS }) {
