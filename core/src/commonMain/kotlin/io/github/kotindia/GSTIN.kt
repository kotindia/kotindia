...
private val GSTIN_ALLOWED_CHARS: Set<Char> =
    buildSet {
        for (c in 'A'..'Z') add(c)
        for (c in 'a'..'z') add(c)
        for (c in '0'..'9') add(c)
...
public object GSTIN {
    ...
    /**
     * Maximum accepted length for a sanitized GSTIN input.
     *
     * GSTIN is always 15 characters. Any sanitized input longer than this is over-length.
     */
    public val maxLength: Int = 15

    /**
     * Set of characters accepted by [sanitize] and [validateProgressive] for GSTIN.
     *
     * GSTIN accepts ASCII uppercase letters (`'A'..'Z'`), lowercase letters (`'a'..'z'`),
     ...
     * directly to `if (char in GSTIN.allowedChars)` UI filters without pre-uppercasing. The
     ...
     */
    public val allowedChars: Set<Char> = GSTIN_ALLOWED_CHARS
     ...
     * @sample io.github.kotindia.samples.gstinSanitizeSample
     */
    public fun sanitize(rawInput: String): String = rawInput.filter { it in GSTIN_ALLOWED_CHARS }.take(maxLength)

    /**
     * Validates a GSTIN for incremental, as-you-type input.
     *
     ...
     *
     * @param value GSTIN input to evaluate, possibly partial. Callers should run [sanitize]
     ...
     * @sample io.github.kotindia.samples.gstinValidateProgressiveSample
     */
    public fun validateProgressive(value: String): ProgressiveResult {
        ...
        if (normalized.any { it !in GSTIN_ALLOWED_CHARS }) {
    ...
    // GSTIN has no standardised spacing — uppercase as-is.
