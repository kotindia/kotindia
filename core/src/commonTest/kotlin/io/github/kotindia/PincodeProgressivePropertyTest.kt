...
/**
 * Property-based tests for [Pincode] progressive validation API.
 */
class PincodeProgressivePropertyTest {
    private val arbPartialDigits: Arb<String> =
        Arb.list(Arb.element(('0'..'9').toList()), 1..5).map { it.joinToString("") }
    ...
    fun pt2_sanitize_idempotent() {
        runBlocking {
            checkAll(1000, arbArbitraryShort) { input ->
                val once = Pincode.sanitize(input)
                assertEquals(once, Pincode.sanitize(once), "sanitize not idempotent: input=$input")
            }
        }
    }
    ...
    fun pt3_sanitize_boundsAndCharset() {
        runBlocking {
            checkAll(1000, arbArbitraryLong) { input ->
                val result = Pincode.sanitize(input)
                assertTrue(result.length <= Pincode.maxLength)
                for (c in result) assertTrue(c in Pincode.allowedChars, "disallowed char '$c' in: $result")
            }
        }
    }
}
